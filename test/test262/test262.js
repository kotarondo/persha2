...
var PromiseContext = require('promise_context')
var fs = require('fs')
...
ctx.loop("main:", function() {
    ...
    ctx.chain(function(resolve, reject) {
        fs.readFile(filename, function(err, data) {
            if (err) {
                reject("cannot read: " + filename);
                return;
            }
            resolve(JSON.parse(data).testsCollection.tests);
        });
    });
    ctx.loop(function(tests) {
